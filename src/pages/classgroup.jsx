  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-purple-600 to-pink-400 text-white">
      <div className="container mx-auto px-6 py-8">
        ...
        <div
  className={`grid gap-6 h-[calc(100vh-140px)] md:h-[calc(100vh-180px)] transition-all duration-500 ${
    selectedGroup ? 'grid-cols-1' : 'lg:grid-cols-3'
  }`}
>
  {/* Left Column: Class Groups */}
  {!selectedGroup && (
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {classGroups.map((group) => (
            ...
            >
              <h3 className="font-bold text-lg mb-1">{group.faculty_name}</h3>
              <p className="text-sm text-purple-200">
                Room: {group.room_number} | Slot: {group.slot}
              </p>
  ...
  <div
    className={`bg-black bg-opacity-30 backdrop-blur-sm rounded-2xl border-2 border-purple-300 flex flex-col overflow-hidden transition-all duration-500 ${
      selectedGroup ? 'col-span-1' : 'lg:col-span-2'
    }`}
  >
    {selectedGroup ? (
      <>
        {/* Chat Header */}
        <div className="p-6 border-b border-purple-300 flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-bold">{selectedGroup.faculty_name}</h2>
            <p className="text-purple-200 text-sm">
              {selectedGroup.building_id.replace('building-', 'Building ').toUpperCase()} - 
              Room {selectedGroup.room_number} - Slot {selectedGroup.slot}
            </p>
          </div>
          {/* Back button to go to group list */}
          <button
            onClick={() => setSelectedGroup(null)}
            className="text-white bg-purple-700 px-4 py-2 rounded-full hover:bg-purple-800 transition"
          >
            ← Back
          </button>
        </div>
